...
public class UserService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        //模拟数据库中的密码，后期需要查询数据库
        String passwordInDb = "{noop}" + userInDb.getPassword();//`对明文的密码进行加密

        List<GrantedAuthority> list = new ArrayList<>();//固定写法
        ...
        //校验 交给 框架
        UserDetails user = new User(username,passwordInDb,list);//用户名,密码,权限
        return user;
    }
}